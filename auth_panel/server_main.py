...
def log_message(message):
    ...
    if HEADLESS_MODE:
        print(f'[{total_logged - 1}] {safe_message}', flush=True)
...
async def handle_client(client):
    ...
    try:
        while True:
            ...
            # ---
            ...
            elif request.startswith("ADMIN_STATUS::"):
                ...
                if not admin_token or not secrets.compare_digest(provided_token, admin_token):
                    ...
                    response = "ADMIN_STATUS::" + json.dumps(payload)
...
def init():
    ...
    if HEADLESS_MODE:
        log_message("Running in headless mode — no console UI")
        status = "Running (headless)"
        try:
            threading.Event().wait()
        except KeyboardInterrupt:
            log_message("Headless server interrupted")
            status = "Exiting..."
    else:
